...
fixed_noise = torch.randn(1, 200, 1).to("cpu")
...
gen = EEG_PRO_GAN_Generator(200, 50, SIGNAL_SIZES[0], factors=FACTORS, signal_channels=22).to("cpu")
checkpoint = torch.load("models/ALL_generator_checkpoint_6_250.pth", map_location=torch.device('cpu'))
...
for batch_idx, (real, _) in enumerate(loader):
    ...
    for i in range(4):
        ...
        axs[i+4].plot(fake[0, i, :].cpu().detach().numpy())
